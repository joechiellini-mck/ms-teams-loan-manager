...
@PreventIframe("/requestApprovalMessageExtension/config.html")
export default class RequestApprovalMessageExtension implements IMessagingExtensionMiddlewareProcessor {
    ...
    public async onQuerySettingsUrl(context: TurnContext): Promise<{ title: string, value: string }> {
        return Promise.resolve({
            title: "Request Approval Configuration",
            value: `https://${process.env.HOSTNAME}/requestApprovalMessageExtension/config.html?name={loginHint}&tenant={tid}&group={groupId}&theme={theme}`
        });
    }
